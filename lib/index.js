  ...
  (0,_hooks_UseEventHandler__WEBPACK_IMPORTED_MODULE_3__["default"])(rtcChannel, 'new_message', setLoading, message => {
    ...
    if (content?.ctx && Object.keys(content?.ctx)?.length) {
      if (props.annotationTool === _constants__WEBPACK_IMPORTED_MODULE_10__.ANNOTATION_TOOLS.pen) {
        (0,_utils__WEBPACK_IMPORTED_MODULE_9__.initializeAnnotation)('pen', canvasCtx, canvasRef);
...
const Annotation = _ref => {
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    ...
    const handleResize = () => {
      ...
      (0,_utils__WEBPACK_IMPORTED_MODULE_2__.redraw)(context, canvasRef, annotation); // Redraw existing drawings based on new size
    };
  ...
  const startDrawing = e => {
    ...
    if (currentTool === _constants__WEBPACK_IMPORTED_MODULE_1__.ANNOTATION_TOOLS.pen) {
      ...
      setAnnotation(prev => [...(Array.isArray(prev) ? prev : []), {
        type: 'currentCircle',
        ctx: canvasCtx,
        circle: {
          x: centerXPercent,
  ...
  const draw = e => {
    if (currentTool === _constants__WEBPACK_IMPORTED_MODULE_1__.ANNOTATION_TOOLS.pen && drawing) {
      ...
      (0,_utils__WEBPACK_IMPORTED_MODULE_2__.redraw)(canvasCtx, canvasRef, annotation); // Redraw everything on each mouse move to update the circle
    }
  };
  const stopDrawing = () => {
    if (currentTool === _constants__WEBPACK_IMPORTED_MODULE_1__.ANNOTATION_TOOLS.pen && drawing) {
  ...
  const placeEmoji = e => {
    ...
    (0,_utils__WEBPACK_IMPORTED_MODULE_2__.redraw)(canvasCtx, canvasRef, [...annotation, {
      type: 'emoji',
      ctx: canvasCtx,
      emoji: {
        x: xPercent,
    ...
    }]); // Redraw to immediately show the emoji
    if (channel) {
      pushMessage(JSON.stringify({
        ctx: canvasCtx,
        emoji: {
...
const initializeAnnotation = (type, canvasCtx, canvasRef) => {
  const {
    offsetX,
    offsetY
...
const drawAnnotation = (type, canvasCtx, canvasRef) => {
  const {
    offsetX,
    offsetY
  } = getCanvasPosition(e, canvasRef);
