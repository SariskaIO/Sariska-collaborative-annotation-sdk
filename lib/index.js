...
const Canvas = _ref => {
  ...
  const [mode, setMode] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(inputProps.annotationTool); // Modes: "freehand", "circle", "emoji"
  const [paths, setPaths] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)([]); // Store freehand paths
  const [circles, setCircles] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)([]); // Store circles
  const [emojis, setEmojis] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)([]); // Store emojis
  const [currentPath, setCurrentPath] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)([]); // Store the current path being drawn
  const [startPos, setStartPos] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(null); // For circle: starting position
  const [emoji, setEmoji] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)("😊"); // Default emoji
  ...
  const handleResize = () => {
    ...
    const scaledPaths = paths.map(path => path.map(_ref2 => {
      let [x, y] = _ref2;
  ...
  const startDrawing = _ref3 => {
    ...
    setDrawing(true);
    if (mode === _constants__WEBPACK_IMPORTED_MODULE_2__.ANNOTATION_TOOLS.pen) {
      setCurrentPath([[offsetX, offsetY]]);
    } else if (mode === _constants__WEBPACK_IMPORTED_MODULE_2__.ANNOTATION_TOOLS.circle) {
      setStartPos({
        x: offsetX,
        y: offsetY
      });
    } else if (mode === _constants__WEBPACK_IMPORTED_MODULE_2__.ANNOTATION_TOOLS.emoji) {
  ...
  const draw = _ref4 => {
    ...
    } else if (mode === _constants__WEBPACK_IMPORTED_MODULE_2__.ANNOTATION_TOOLS.circle && startPos) {
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    ...
    if (currentPath.length > 0) {
      ctx.beginPath();
      currentPath.forEach((_ref9, index) => {
        let [x, y] = _ref9;
        if (index === 0) {
          ctx.moveTo(x, y);
        } else {
