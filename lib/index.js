...
const App = props => {
  ...
  const roomName = props.meetingTitle || (0,_utils__WEBPACK_IMPORTED_MODULE_9__.getRoomName)();
  const {
    users,
    dispatch
...
const Canvas = _ref => {
  ...
  const textboxHook = (0,_hooks_useOnTextBox__WEBPACK_IMPORTED_MODULE_5__.useOnTextBox)(pushMessage, channel, setCanvasCtx, annotations, setAnnotations, otherProps);
  ...
  const {
    ...
    handleTextChange,
    textboxes
  } = inputProps.annotationTool === _constants__WEBPACK_IMPORTED_MODULE_2__.ANNOTATION_TOOLS.emoji ? emojiHook : inputProps.annotationTool === _constants__WEBPACK_IMPORTED_MODULE_2__.ANNOTATION_TOOLS.circle ? circleHook : inputProps.annotationTool === _constants__WEBPACK_IMPORTED_MODULE_2__.ANNOTATION_TOOLS.textbox ? textboxHook : drawHook;
  const canvasStyle = {
    position: 'absolute',
    zIndex: otherProps.zIndex,
    background: 'none'
  };
  let drawnTextboxes = annotations?.length && annotations?.filter(annotation => annotation.type === _constants__WEBPACK_IMPORTED_MODULE_2__.ANNOTATION_TOOLS.textbox.toLowerCase())?.map(annotation => annotation.textbox);
