...
function useSticker(pushMessage, channel, setCanvasCtx, otherProps) {
  ...
  const [isButtonEnabled, setIsButtonEnabled] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(false); // State to track if the button is enabled
  ...
  }, [setCanvasCtx, otherProps.isCanvasClear]);
  function setStickerCanvasRef(ref) {
    if (!ref) return;
    canvasRef.current = ref;
  }
  function handleClick(e) {
    ...
    const ctx = canvasRef.current.getContext('2d');
    const point = (0,_utils__WEBPACK_IMPORTED_MODULE_1__.computePointInCanvas)(e.clientX, e.clientY, canvasRef.current); // Get cursor point
